...
class PhpRenderer extends RendererAbstract
{
    ...
    public function enterObjectRefClass(ObjectRefClass $node)
    {
        ...
        $this->raw('ThinkHaml\Runtime::renderObjectRefClass(');
    ...
    public function enterObjectRefId(ObjectRefId $node)
    {
        ...
        $this->raw('ThinkHaml\Runtime::renderObjectRefId(');
    ...
    public function enterFilter(Filter $node)
    {
        ...
        if (!$filter->isOptimizable($this, $node, $this->env->getOptions())) {
            $this->pushEchoMode(false);
            $this->write('<?php echo ThinkHaml\Runtime::filter('.$this->env->getOption('mthaml_variable').', '.var_export($node->getFilter(), true).', get_defined_vars(),');
            $this->indent();
    ...
    {
        $n = 0;
        if('include' == $tag->getTagName()) {
            //$this->raw(' file='.$tag->getAttributes()[0]->getValue()->getContent().' /');
            foreach ($tag->getAttributes() as $attr) {

                $this->raw(' '.$attr->getName()->getContent().'='.$attr->getValue()->getContent());
            }
        } else {
            $this->raw(' <?php echo ThinkHaml\Runtime::renderAttributes(array(');
            ...
            foreach ($tag->getAttributes() as $attr) {
                ...
                if ($attr instanceof TagAttributeInterpolation) {
                    $this->raw('ThinkHaml\Runtime\AttributeInterpolation::create(');
                    $attr->getValue()->accept($this);
                    $this->raw(')');
                } else if ($attr instanceof TagAttributeList) {
                    $this->raw('ThinkHaml\Runtime\AttributeList::create(');
                    $attr->getValue()->accept($this);
                    $this->raw(')');
                } else {
                    $this->raw('array(');
